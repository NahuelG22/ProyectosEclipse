...
float promediar(int contador1, int contador2)
{
    ...
    {
        retorno=(float)contador1/contador2;
    }
		...
		switch(opcion)
		{
			...
			case 3:
				...
				else
				{
				    ...
				    if(contadorUEFA>contadorConcacaf && contadorUEFA>contadorConmebol && contadorUEFA>contadorCAF && contadorUEFA>contadorAFC && contadorUEFA>contadorOFC)
				    {
				        aumentoUEFA=acumuladorCostoMantenimiento*35/100;
				        acumuladorCostoMantenimientoAumento=acumuladorCostoMantenimiento+aumentoUEFA;
				    }
				    else
				    {
				    	acumuladorCostoMantenimientoAumento=acumuladorCostoMantenimiento;
				    }
					if(ingresoTres==0)
    				{
    					ingresoTres=1;
